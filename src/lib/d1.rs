pub fn d1_anser(cal_val: &str) -> u32 {
    ...
    String::from(cal_val)
        ...
        .fold(0 as u32, |mut total, (cur1, cur2)| {
            ...
            total = total
                + (format!("{}{}",temp.unwrap(), temp2.unwrap())).parse::<u32>().unwrap();
...
3nine6five1"#;
        println!("{:?}", testdata);
        let ansr = d1_anser(testdata);

        println!("{:?}", ansr);
        assert_eq!(ansr, 142 as u32)
    }
}
