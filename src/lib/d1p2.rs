...
pub fn d1p2_anser(cal_val: &str) -> u32 {
    ...
    String::from(cal_val)
        .lines()
        .zip(cal_val.lines().map(|x| x.chars().rev().collect::<String>()))
        .fold(0 as u32, |mut total, (cur1, cur2)| {
            let mut temp: char = char::from(' ');
            let mut temp2: char = char::from(' ');

            'outer: for (index, character) in cur1.chars().enumerate() {
                let mut m = String::from("");
                if char_map.contains_key(&character) {
                    temp = character.clone();
                    break 'outer;
                }
                for index_2 in index..(std::cmp::min(index + 5, cur2.len())) {
                    m.push(cur1.chars().nth(index_2).unwrap());
                    if map.contains_key(m.as_str()) {
                        temp = map.get(m.as_str()).unwrap().clone();
                        break 'outer;
                    }
                }
            }

            'outer: for (index, character) in cur2.chars().enumerate() {
                let mut m = String::from("");
                if char_map.contains_key(&character) {
                    temp2 = character.clone();
                    break 'outer;
                }
                for index_2 in index..(std::cmp::min(index + 5, cur2.len())) {
                    m.push(cur2.chars().nth(index_2).unwrap());
                    if map.contains_key(m.as_str()) {
                        temp2 = map.get(m.as_str()).unwrap().clone();
                        break 'outer;
                    }
                }
            }

            println!("HERE IS OUR INTS: {}{}", temp, temp2);

            total = total + (format!("{}{}", temp, temp2)).parse::<u32>().unwrap();
...
pub mod test {
    ...
    #[test]
    pub fn d1_test() {
        let testdata = r#"ckmb52fldxkseven3fkjgcbzmnr7
